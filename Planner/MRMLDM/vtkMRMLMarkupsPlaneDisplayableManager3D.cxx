...
public:
  ...
  typedef std::map <vtkImplicitPlaneWidget2*, Pipeline* > WidgetToPipelineMapType;
...
//---------------------------------------------------------------------------
vtkImplicitPlaneWidget2* vtkMRMLMarkupsPlaneDisplayableManager3D::vtkInternal
::CreatePlaneWidget() const
{
                       ...
                       Pipeline* pipeline)
{
  ...
  if (planeNode && planeNode->GetNumberOfMarkups() > numberOfWidgets)
    ...
    for (int i = numberOfWidgets; i < planeNode->GetNumberOfMarkups(); ++i)
      {
      vtkSmartPointer<vtkImplicitPlaneWidget2> widget = this->CreatePlaneWidget();
      pipeline->Widgets.push_back(widget);
      this->WidgetMap[widget] = pipeline;
      }
    }
  ...
  for (size_t i = 0; i < pipeline->Widgets.size(); ++i)
    ...
    vtkImplicitPlaneWidget2* widget =
      vtkImplicitPlaneWidget2::SafeDownCast(pipeline->Widgets[i]);
    bool visible = planeNode ? planeNode->GetNthMarkupVisibility(i) : false;
...
//---------------------------------------------------------------------------
void vtkMRMLMarkupsPlaneDisplayableManager3D::vtkInternal
::UpdateNodeFromWidget(vtkImplicitPlaneWidget2* planeWidget)
{
  assert(widget);
  Pipeline* pipeline = this->WidgetMap.at(planeWidget);
  assert(pipeline);
...
::ProcessMRMLNodesEvents(vtkObject* caller, unsigned long event, void* callData)
{
  ...
  if (planeNode)
    {
    vtkMRMLDisplayNode* callDataNode = reinterpret_cast<vtkMRMLDisplayNode *>(callData);
    displayNode = vtkMRMLMarkupsDisplayNode::SafeDownCast(callDataNode);
